...
fn upload_json_file(file_content: Vec<u8>) {
    let mut some_state: Option<State> = None;
        STATE_MAP.with(|map| map.borrow_mut().get(&String::from("state")));
    if some_state.clone().is_none() {
        some_state= Some(State::default());
        STATE_MAP.with(|map| {
            map.borrow_mut().insert(String::from("state"), some_state.clone().unwrap());
        });
    }
    ...
    let mut state=some_state.unwrap();
...
async fn train(epochs: u64) {
    let initial_cycles = ic_cdk::api::canister_balance();
    ic_cdk::println!("cycles used start:{}", initial_cycles);
    // 检查并填充 RANDOM_BUFFER
...
fn predict() -> f32 {
    ...
    ic_cdk::println!("predict cycles balance: {}", ic_cdk::api::canister_balance());
    denormalize(
        output.into_scalar(),
        state.min_values[3],
        state.max_values[3],
