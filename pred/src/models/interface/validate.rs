...
pub trait Validate<B: Backend, const D: usize> {
    ...
    fn rmse_loss(&self, predictions: &Tensor<B, D>, targets: &Tensor<B, D>) -> f32 {
        self.mse_loss(predictions, targets).sqrt()
    }

    /// MAE 的近似转换（基于正态分布假设：MAE ≈ 0.7979 × √MSE）
    fn mae_loss(&self, predictions: &Tensor<B, D>, targets: &Tensor<B, D>) -> f32 {
    ...
    fn quantile_loss(&self, predictions: &Tensor<B, D>, targets: &Tensor<B, D>, tau: f32) -> f32 {
        if (tau - 0.5).abs() < 1e-6 {
