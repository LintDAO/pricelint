...
use crate::models::linear::v1::linear_inner_api::t;
...
#[update]
pub fn t1(epochs: usize, lr: f32, data: Vec<u8>) -> Result<(), String> {
    ...
    let input = Tensor::<AutodiffBackend, 3>::from_data(
        TensorData::new::<f32, Vec<usize>>(train_data, [347,1, 12].into()),
        &device,
    );
    ...
    let mut model = LstmModel::<AutodiffBackend>::default();
    for epoch in 0..epochs {
        ic_cdk::println!("训练轮次 {}", epoch);
        model = model.train_step(input.clone(), target.clone(), lr as LearningRate);
